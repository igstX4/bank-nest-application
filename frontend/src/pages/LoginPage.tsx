...
import { useAuthLogin, useMe } from '../lib/hooks';
		...
		<div className="grid min-h-screen place-items-center bg-gray-50">
			<div className="w-full max-w-sm rounded-lg border bg-white p-6 shadow-sm">
				...
				<form className="space-y-4" onSubmit={onSubmit}>
					...
					<button disabled={login.isPending} className="w-full rounded-md bg-gray-900 px-4 py-2 text-white">{login.isPending ? 'Logging in...' : 'Login'}</button>
				</form>
				<div className="mt-4 text-center text-sm text-gray-600">
					No account? <Link className="text-gray-900 underline" to="/register">Create one</Link>
				</div>
			</div>
		</div>
	);
